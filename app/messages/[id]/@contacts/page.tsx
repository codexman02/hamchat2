...
//    let [messages,setMessages]=useState<any | null>(null);
   ...
   let [data,setData]=useState<any | null>(null)
    ...
    let socketName="_contacts"+id;
     let [checkAgain,setCheckAgain]=useState(false)
    ...
    function checkUserId(){
        setTimeout(()=>{
            if(session.status=="loading"){
                ...
                if(session.data){
                    if(id!==session.data?.user._id){
                        router.push("/")
                    }else if(!session.data.user.isVerified){
                        router.push(`/profile/${session!.data.user._id}`)
                    }
                }
            }else{
                router.push("/")
...
let router=useRouter();
    async function test(){
        ...
        // let messages =y?.messages?.list 
        // let contacts=await getContacts(messages)
        // console.log(contacts)
        // console.log(messages)
        // setMessages(y?.isMessagesSchema);
        // let data:UserData=await JSON.parse(y)
        if(y){
            ...
            let sortedContacts=contacts.sort((a,b)=>{
                let first:any=new Date(a.lastMessage.sentAt);
                let second:any=new Date(b.lastMessage.sentAt) ;
                // console.log(first,second,"firts and scenfs")
                return second-first
                // return second-first
            })
...
{data.length>0? (data.map((ele:{username:string,contact_id:string,email:string,lastMessage:{content:string,sentAt:Date},profile:string},i:number)=>( 
    <div key={i}> 
     {/* <Link href={`/messages/${id}/chats/${ele.contact_id}`} key={ele.username} className="relative z-10"> */}
    {ele.contact_id==chatId?(
        <>
         <div className="list-items w-full px-3 py-5 flex relative bg-gray-200" key={ele.username}>
         ...
         <div className="flex flex-col w-full overflow-hidden text-nowrap text-ellipsis ml-5 grow">
             <h2 className="text-xl font-semibold">{ele.username}</h2>
         <p className="w-full py-1 content-center text-lg overflow-x-hidden text-nowrap text-ellipsis">{ele?.lastMessage.content}</p>
         </div>
         </Link>
         ...
         <div className="delete flex justify-center items-center w-1/12">
 ...
 <div className="list-items px-3 py-5 flex relative bg-white" key={ele.username}>
             <Link href={`/messages/${id}/chats/${ele.contact_id}`} key={ele.username} className="relative z-10 flex w-11/12">
